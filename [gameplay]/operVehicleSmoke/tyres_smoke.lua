...
local function addVehicleSmoke(vehicle)
	...
	if not VehiclesProperty.driftsmoke then
		return
	end
	...
	vehicle:setData("SmokeColor", {155, 155, 155})
	return true
end
...
addEventHandler("onClientElementDataChange", root, function(data, _, state)
	...
	if data == 'vehicle.isProperty' and state ~= nil then
		if state.driftsmoke then
			addVehicleSmoke(source)
		else
			removeVehicleSmoke(source)
		end
...
addEvent("operVehicleSmoke.update", true)
addEventHandler("operVehicleSmoke.update", root, function(state)
	for i, v in ipairs(getElementsByType("vehicle")) do
		if state and isElementStreamedIn(v) then
			addVehicleSmoke(v)
		else
