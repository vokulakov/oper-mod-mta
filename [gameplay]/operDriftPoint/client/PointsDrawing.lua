...
function PointsDrawing.draw()
	...
	if not PointsDrawing.isVisible then
		return
	end
	...
	if not PLAYER_UI['drift_point'] then return end
...
addEventHandler("onClientResourceStart", resourceRoot, function ()
	font = dxCreateFont("Roboto-Regular.ttf", FONT_SIZE, true)
	font2 = dxCreateFont("Roboto-Regular.ttf", MULTIPLIER_FONT_SIZE, true)
...
addEvent('operDriftPoint.setVisible', true)
addEventHandler('operDriftPoint.setVisible', root, function(state)
	PointsDrawing.isVisible = state or not PointsDrawing.isVisible
end)