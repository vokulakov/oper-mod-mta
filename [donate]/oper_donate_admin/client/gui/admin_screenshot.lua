...
function aPlayerScreenShot (player)
	if aScreenShotForm == nil then
		...
		aScreenShotForm		= guiCreateWindow	( x / 2 - 300, y / 2 - 125, 600, 250, "Управление скриншота", false )
		aScreenShotList		= guiCreateGridList	( 0.03, 0.08, 0.70, 0.90, true, aScreenShotForm )
		aScreenShotNew		= guiCreateButton	( 0.75, 0.08, 0.42, 0.09, "Посмотреть новые", true, aScreenShotForm )
		aScreenShotDelete	= guiCreateButton	( 0.75, 0.18, 0.42, 0.09, "Удалить", true, aScreenShotForm )
		aScreenShotView		= guiCreateButton	( 0.75, 0.28, 0.42, 0.09, "Посмотреть", true, aScreenShotForm )
		aScreenShotRefresh	= guiCreateButton	( 0.75, 0.38, 0.42, 0.09, "Обновить", true, aScreenShotForm )
		aScreenShotClose	= guiCreateButton	( 0.75, 0.88, 0.42, 0.09, "Закрыть", true, aScreenShotForm )
		guiGridListAddColumn(aScreenShotList,"Игрок",0.31 )
		guiGridListAddColumn(aScreenShotList,"Админ",0.31 )
		guiGridListAddColumn(aScreenShotList,"Дата",0.27 )
...
addEventHandler("aClientScreenShot",resourceRoot,
	function (action,player,data,arg1,arg2,arg3)
		if action == "new" then
			...
			aScreenShotWindows[player].label = guiCreateLabel(0,0,1,1,"Загрузка...",true,aScreenShotWindows[player].window)
