...
function createMapTab()
	...
	aTabMap.Revert = guiCreateButton ( 0.45, 0.45, 0.3, 0.04, "Возвратить карту", true, aTabMap.Tab )
	guiSetVisible(aTabMap.Delete, false)
	guiSetVisible(aTabMap.Revert, false)
	aTabMap.RefreshList = guiCreateButton ( 0.03, 0.91, 0.35, 0.04, "Обновить лист", true, aTabMap.Tab )
	...
	if button == "left" then
		if ( getElementParent ( source ) == aTabMap.Tab ) then
			...
			elseif source == aTabMap.Delete then
				aMessageBox ( "Вопрос", "Вы уверены в удалении '"..mapName.."'?", "triggerServerEvent(\"deleteRevertMap_s\", getLocalPlayer(), true, \""..mapResName.."\", \""..mapName.."\")" )
